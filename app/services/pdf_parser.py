...
class ResumePDFParseError(ValueError):
    """Raised when the uploaded PDF cannot be parsed into readable text."""


def extract_resume_text(pdf_bytes: bytes) -> str:
    # Normalize extracted text and reject empty/scanned PDFs early.
    if not pdf_bytes:
        raise ResumePDFParseError("The uploaded PDF is empty.")

    text = _extract_with_pdfplumber(pdf_bytes) or _extract_with_pypdf2(pdf_bytes)
    normalized = "\n".join(line.strip() for line in text.splitlines() if line.strip())

    if len(normalized) < 50:
        raise ResumePDFParseError("Could not extract enough text from the PDF resume.")
